...
test("processFiles", async t => {
  const MEDIA_PREFIX = "https://wibble.com";
  const processor = processFiles({ mediaPrefix: MEDIA_PREFIX });
  const updates = await processor([FILE_1, FILE_2, FILE_3]);
  ...
  //t.same(typeof updates[0].keywords, "object", "adds keywords");
