...
const fetchTwitterOpenGraph = async (url) => {
  const browser = await puppeteer.launch();
  const page = await browser.newPage();
  await page.goto(url);
  await page.waitForSelector('[data-testid="tweet"]', { visible: true, timeout: 10000 });
  ...
  return Promise.all(
    update.links.map(url => {
      ...
      if (/twitter\.com/.test(url)) {
        console.error("Twitter link detected. Rendering page: " + url);
        promise = fetchTwitterOpenGraph(url);
      } else {
        promise = ogs({ url, followAllRedirects: true })
      }
